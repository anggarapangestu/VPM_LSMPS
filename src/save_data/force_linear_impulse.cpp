...
void save_data::set_variables(int numberofparticles_x, int numberofparticles_y, int totalnumberofparticles){
...
void save_data::save_dat(Particle &p, int np, vector<int> &numbar, vector<int> &batas_bawah, vector<int> &batas_atas, 
...
void save_data::force_linear_impulse(int it, int np, const vector<double> &xp, const vector<double> &yp,
                                     const vector<double> &gpz, const vector<double> &sp)
{
    ...
    // A_ref[1] = Pars::lx;
    // =========== Linear Impulse ===============================
    this->_Isumx.push_back(0.0e0); // accessed from (a)
    ...
    for (int i = 0; i < np; i++)
    {
        ...
        double _area = std::pow(sp[i] / Pars::sigma, 2);
        //_area = pow(sp[i], 2);
        //gpz -> vorticity
...
//=========================================================================================================================
void save_data::Force2(int iT, double A1, double A2, double A3, double A4, int nx, int ny, Particle &p)
{
    //box size
    ...
    if (iT != 0){
        ...
        for (int i = 0; i < this->point_lists.size();i++){
    ...
    for (int i = 0; i < this->particles_inside.size(); i++){
        int ii = i;
	...
	{
		Cd2 = 2.0*FX/((Pars::U_inf*Pars::U_inf)*Pars::lx);
		Cl2 = 2.0*FY/((Pars::U_inf*Pars::U_inf)*Pars::lx);
	}
...
void save_data::sep(Particle p, std::vector<double> a, std::vector<double> b, std::vector<double> c, std::vector<double> d, std::vector<double> e, string s){
    std::ofstream ofs;
    string nama;
