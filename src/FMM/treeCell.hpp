...
struct TreeCell{
    ...
    */
    ...
    const int max_source;       // Maximum of source particle inside the cell
    int cellCount;              // Number cell throughout the tree
    double maxDomain[DIM];      // [Constant upon set up] Domain extreme maximum boundary
    double minDomain[DIM];      // [Constant upon set up] Domain extreme minimum boundary
    ...
    int get_indexCount(const int &_level) const;            // Get the number of cell at current level (the number at one direction only)
    ...
    bool inside_domain(const int &_ID) const;               // Check the given cell is inside the domain
    ...
    // Hierarchy function
    int get_parentID(const int &_ID) const;
    std::vector<int> get_childID(const int &_ID) const;
    ...
    TreeCell() : 
        ...
        max_particle(Pars::par_count_max),  // Maximum number of particle inside the cell
        max_source(Pars::src_count_max),    // Maximum number of source particle inside the cell
