...
void pressure_poisson::get_pressure(Particle& p){
	...
	bool cp_calc = true;	// Toogle for the pressure coefficient calculation
	bool src_save = true;	// Toogle for saving the pressure source
	...
	std::vector<double> phi;		// phi = P/rho (type 1) or H (type 2)
	FMM_step.get_Potential(phi);
	...
	if (cp_calc == true){
		...
		neighbor d_base_ngh;
		d_base_ngh.inter_search(buffer, surface, _par2grdNeighbor);
		d_base_ngh.inter_search(surface, surface, _grd2grdNeighbor);
