...
void CellList::initCellList(const Particle & particle){
    ...
    */
    ...
    double *min_lim = new double[Pars::DIM];
    double *max_lim = new double[Pars::DIM];
    ...
    this->mid_pos.resize(this->num, std::vector<std::vector<double>>(this->cellNum,std::vector<double>(Pars::DIM,0.0)));
    ...
    this->center_pos.resize(Pars::DIM,0);
    this->pivot_pos.resize(Pars::DIM,0);
    ...
    std::vector<int> _ind = std::vector<int>(Pars::DIM,0);
    ...
    this->index.resize(this->cellNum,std::vector<int>(Pars::DIM,0));
    this->lvl.resize(this->cellNum);
    if (Pars::DIM == 2){        this->T_map2D.resize(Pars::max_level + 1);}
    else if (Pars::DIM == 3){   this->T_map3D.resize(Pars::max_level + 1);}

    // Initialize from the root cell (ID = 0)
    if (Pars::DIM == 2){        this->T_map2D[0] = std::vector<std::vector<int>>(1,std::vector<int>(1,0));}
    else if (Pars::DIM == 3){   this->T_map3D[0] = std::vector<std::vector<std::vector<int>>>(1,std::vector<std::vector<int>>(1,std::vector<int>(1,0)));}
    this->lvl[0] = 0;
    this->index[0] = std::vector<int>(Pars::DIM,0);
...
void CellList::createCellList(Particle & particle)
{
    ...
    */
    ...
    // Internal variables
    std::vector<int> x_cell_pos (Pars::DIM,0);
    ...
    for (size_t i = 0; i < this->num; i ++){
        ...
        while(true){
            ...
            std::vector<int> idx = std::vector<int>(Pars::DIM,0);   // Initialize the index counter
            for (size_t j = 0; j < this->basis; j++){               // does work for 3D
                // Update the counter
                for (int e = 0; e < Pars::DIM - 1; e ++){
                    idx[e+1] += idx[e] / 2;
                    idx[e] = idx[e] % 2;
