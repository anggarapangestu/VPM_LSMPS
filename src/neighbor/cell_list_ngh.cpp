...
void CellList::neighborCell (std::vector<std::vector<int>>& ngh_PAIR_basis_cell_ID, int basis_ID, int cell_ID){
    ...
    */
    ...
    std::vector<int> idx(Pars::DIM,0);      // Trinary index for neighbor ID shift
    std::vector<int> mat_id(Pars::DIM,0);   // The candidate neighbor transformation matrix index
    std::vector<int> cg_id(Pars::DIM,0);    // The basis ID shift (-1: Prior Box, 0: Current Box, 1: Next Box)
    std::vector<int> cg_basis(Pars::DIM,0); // The basis ID loc. mark (-1: Lowest Bound, 0: Middle, 1: Highest Bound)
    int temp_ngh_basis;
    int temp_ngh_cell;
    ...
    for (int i = 0; i < temp_ngh_PAIR_list.size(); i ++){
...
{   
    ...
    */
    ...
    // #=============== LOOPS 1 ===============#
    for (size_t id = 0; id < particle.num; id++){
