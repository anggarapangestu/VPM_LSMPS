...
private:
    ...
    const int basis = std::pow(2,Pars::DIM);        // Number of division child cell for each DnC
    const int basisNGH = std::pow(3,Pars::DIM);     // The basis used for neighbor evaluation
...
public:
  ...
  void find_neighborcells(int i_cell, int j_cell, int neighbor_scale, int nGx, int nGy,
