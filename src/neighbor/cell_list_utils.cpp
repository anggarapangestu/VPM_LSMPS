...
int CellList::findParent(int CHD_ID){
    int parID;
    parID = std::ceil(CHD_ID/std::pow(2,Pars::DIM)) - 1;
    return parID;
}
...
void CellList::saveCellData(){
    ...
    for (int i = 0; i < this->mid_pos.size(); i++){
        for (int j = 0; j < this->mid_pos[i].size(); j++){
...
void CellList::checkNGH(const Particle & par){
    ...
    bool _break = false;
    for (int i = 0; i < par.neighbor.size(); i ++){ 
        for (int j = 0; j < par.neighbor[i].size(); j ++){
            for (int k = 0; k < par.neighbor[i].size(); k ++){
                if (j == k){continue;}
