...
// List of the Initilization function
// ==================================================
void treeCell::initializeTree(std::vector<std::vector<double>>& parPos){
    ...
    */
    ...
    // Internal variable
    double* minPos = new double [Pars::DIM];
    double* maxPos = new double [Pars::DIM];

    // PROCEDURE PRE-1! : Calculating and determine the BASIC PARAMETERs
    // ****************
    this->basis = std::pow(2,Pars::DIM);
    this->max_level = Pars::level_max;
    this->min_level = Pars::level_min;
    this->max_par = Pars::n_max;
    ...
    this->cellPos.resize(this->cellNum,std::vector<double>(Pars::DIM,0.0));    // The center position of the cell
    ...
    // ************
    std::vector<double> cenPos(Pars::DIM,0.0);
    for (int e = 0; e < Pars::DIM; e ++){
...
void treeCell::createTree(std::vector<std::vector<double>>& parPos, std::vector<bool> & activeMark){
    ...
    */
    ...
    // Temporary variable
    std::vector<int> bin_ID(Pars::DIM,0);       // The binary index
    std::vector<double> _pos(Pars::DIM,0.0);    // The intermediate center position
    ...
    while (true){
        ...
        bin_ID = std::vector<int> (Pars::DIM,0);    // Reset the binary index
        for (int i = 0; i < this->basis; i++){
            // Update the value of binary index
            for (int e = 0; e < Pars::DIM - 1; e++){
        ...
        if (CURR_ID == 0){
            ...
            for (auto _ID:CHILD_ID){
                if (parNum[_ID] != parIDList[_ID].size()){
...
void treeCell::updateCell(std::vector<std::vector<double>>& parPos, std::vector<bool> & activeMark){
    ...
    for (size_t i = 0; i < this->leafCellList.size(); i++){
        ...
        if ((this->level[cellID] < this->max_level) && (newNum > this->max_par)){
            ...
            std::vector<int> bin_ID = std::vector<int> (Pars::DIM,0);   // Reset the binary index
...
// List of the neighbor cell search function
// ==================================================
void treeCell::intList(int currID, std::vector<int>& ID_list_1, std::vector<int>& ID_list_3){
    ...
    int ngh_num = std::pow(3,Pars::DIM);    // Max number of initial neighbor cell list
    ...
    // ************
    tri_ID = std::vector<int>(Pars::DIM,0); // Reset the trinary index
    ...
    for (int i = 0; i < ID_list_1_temp.size(); i++){
        ...
        if (this->leafCellMark[nghID] == true){
            ...
            if(this->parNum[nghID] == 0){   // If there is no particle, the leaf cell must be located at the parent cell
                ...
                double min_dist = this->baseLen * ( 1/std::pow(2.0,this->level[_chdIDs[0]]) + 
                                  1/std::pow(2.0,currlvl) ) / 2.0 + Pars::tol_dist;
...
void treeCell::extList(int currID, std::vector<int>& ID_list_2, std::vector<int>& ID_list_4){
    // Reset the neighbor list
    ID_list_2.clear();
    ID_list_4.clear();
    ...
    // Internal variable
    int currlvl = this->level[currID];      // The level of current cell
    int ngh_num = std::pow(3,Pars::DIM);    // Max number of candidate neighbor cell
    ...
    for (int i = 0; i < ID_list_2_temp.size(); i++){
        ...
        if ((this->leafCellMark[nghID] == true) || (this->srcNum[nghID] == 0)){
            ...
            double min_dist = this->baseLen / std::pow(2,currlvl) + Pars::tol_dist;
            for (auto _chdID:_chdIDs){
    ...
    for (int i = 0; i < ID_list_4_temp.size(); i ++){
        ...
        double min_dist = this->baseLen * (1/std::pow(2,this->level[nghID]) +
                            1/std::pow(2,currlvl)) / 2.0 + Pars::tol_dist;
...
void treeCell::createTmap2D(){
    ...
    this->cellIdx.resize(this->cellNum,std::vector<int>(Pars::DIM,0));
    ...
    this->cellIdx[0] = std::vector<int>(Pars::DIM,0);
    ...
    for (int k = 1; k <= this->max_level; k ++){
        ...
        for (int _id = _start; _id <= _fin; _id++){
            ...
            c = std::vector<int>(Pars::DIM,0); // Reset the counter
            int currID;
...
// ==================================================
int treeCell::findParentID(int currID){
    int parID;
    parID = std::floor((currID-1)/this->basis);
    return parID;
}

std::vector<int> treeCell::findChildID(int currID){
...
void treeCell::findLevel(const std::vector<int>& ID_list, std::vector<int>& level){
    ...
    for (int i = 0; i < this->cellNum; i++){
        ...
        data << ",";
            if(this->parNum[i] == 0){
                ...
                for (int j = 1; j < this->parIDList[i].size(); j++){
