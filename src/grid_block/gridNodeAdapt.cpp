...
void save_particle(const Particle& _particle, std::string name){
    std::ofstream writer;

    std::cout << "[!] Write the particle " << name << "\n";

    name = "output/particle_" + name + ".csv";
    ...
    for (int i = 0; i < _particle.num; i ++){
        writer <<  "" << _particle.x[i]
		       << "," << _particle.y[i]
		       << "," << _particle.z[i]
		       << "," << Pars::sigma * std::pow(2, Pars::max_level - _particle.level[i])
		       << "," << _particle.isActive[i]
               << "," << _particle.level[i]
               << "," << _particle.s[i]
               << "\n";
...
bool GridNodeAdapt::get_adaptation(GridNode &baseGrid, const Particle &tempPar, 
        const Particle &currPar, int nghConv, std::vector<std::vector<double>*>& errPredProp,
...
bool GridNodeAdapt::get_adaptation(GridNode &baseGrid, const Particle &tempPar, 
    ...
    // Initial Adaptation Log
    std::cout << "Data before adaptation:\n";
    MESSAGE_LOG << "Total nodes       : " << baseGrid.nodeMap.size() << "\n";
    {
        // Calculates number of leaf node
        int leafCnt = 0;
        for (auto& [_ID, _node] : baseGrid.nodeMap){
            if (_node->isLeaf) leafCnt++;
        }
    MESSAGE_LOG << "Total leaf nodes  : " << leafCnt << "\n";
    }
    MESSAGE_LOG << "Total particle    : " << tempPar.num << "\n";
    ...
    */
    ...
    // MESSAGE_LOG << "Set target level of each node in grid!\n";
    // Set the target resolution level of each node in baseGrid
    std::unordered_map<int,std::vector<int>> gridCont;      // The container for the scattered particle
    this->assignPar2Grid(baseGrid, currPar, gridCont);

    // EVALUATION 2
    // MESSAGE_LOG << "Set target level of each node in grid!\n";
    // Set the target resolution level of each node in baseGrid
    this->targetLevelv2(baseGrid, currPar, errPredProp, gridCont);
    ...
    nPar.neighbor.clear();
    nPar.level.clear();
    nPar.s.clear();         // The only variable that follow the predecesor
    nPar.isActive.clear();  // The second variable that follow the predecesor
    ...
    */
    ...
    for (auto &[_headID,_headFlag] : this->refineList){
        ...
        // An update toward the particle division
        _headNode->parList = gridCont.at(_headID);
        ...
        for (size_t i = 0; i < nodeIDrefine.size(); i++){
            ...
            for (int &_particleID : _evalNode->parList){
                ...
                #if (N_BODY > 0)
                if (currPar.isNearSurface[_particleID]){
                    ...
                    _evalNode->tarResLv = Pars::max_level;
                }
                #endif
            }
            ...
            // [COND 3] Particle generation for the node outside the previous criteria
                ...
                // double _headSize = _headNode->length / this->parNum;
                // this->generateParticle(_evalNode, _headSize);
                double _currSize = _evalNode->length / this->parNum;
                this->generateParticle(_evalNode, _currSize);
        ...
        for (int currLvl = fineLvl; currLvl >= 0; currLvl--){
            ...
            for (int &_ID : IDqueue[currLvl]){
                ...
                if (needRefine){
                    ...
                    // Generate the particle inside each child node (*if not meet the criteria)
                        ...
                        for (int &_chdID : chdIDList){
                            ...
                            // double _headSize = _headNode->length / this->parNum;
                            // this->generateParticle(_currNode, _headSize);
                            double _currSize = _currNode->length / this->parNum;
                            this->generateParticle(_currNode, _currSize);
                ...
                // ** Not need refinement
                    ...
                    // Generate the particle inside the node (only for temporary node)
                    ...
                    double _currSize = _Node->length / this->parNum;
                    this->generateParticle(_Node, _currSize);
    ...
    */
    ...
    for (auto &[_currID, _currFlag] : this->compressList){
        ...
        if (_currFlag){    
            ...
            //     // Divide the particle into new child node
            //     this->tempGrid.divideParticle(_evalNode, nPar, _currChdIDList, 0);
            // }
            ...
            //     // Update the particle size
            //     for (int &_parID : _evalNode->parList) nPar.s[_parID] = _tarSize;
    ...
    for (auto &[_ID,_flag] : this->idleList){
        ...
        // Transfer all particle data from the particle list inside the node
        for (int &_particleID : tempParList){
            nPar.x.push_back(tempPar.x[_particleID]);
            nPar.y.push_back(tempPar.y[_particleID]);
            if (DIM > 2) nPar.z.push_back(tempPar.z[_particleID]);
            nPar.s.push_back(tempPar.s[_particleID]);
            nPar.isActive.push_back(tempPar.isActive[_particleID]);
            nPar.neighbor.push_back(std::vector<int>());
            // if (nghConv == 0){
            //     nPar.neighbor.push_back(tempPar.neighbor[_particleID]);
            //     if (!Pars::flag_ngh_include_self){
            //         // Put the current ID into the neighbor list
            //         nPar.neighbor[nPar.num].push_back(_particleID);
            //     }
            // }
            nPar.level.push_back(tempPar.level[_particleID]);
            nPar.nodeID.push_back(tempPar.nodeID[_particleID]);
            
            // Update the particle list in the node
            _node->parList.push_back(nPar.num);
    ...
    // Calculate the total leaf node
    std::cout << "Data after adaptation:\n";
    MESSAGE_LOG << "Final total nodes      : " << baseGrid.nodeMap.size() << "\n";
    {
        int leafCnt = 0;
        for (auto& [_ID, _node] : baseGrid.nodeMap){
