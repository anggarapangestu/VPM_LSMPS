...
void velocity_biot_savart::gaussian_function(double rij, double sij, double &gaussij)
{
  ...
  // ------------ 2nd order -----------
  gaussij = std::exp(-rij * rij / (2.0e0 * pow(sij, 2))) / (pow(sij, 2) * (2.0e0 * Pars::pi));
}
...
void velocity_biot_savart::regul_func_2d(double rij, double sij, double &q)
{
  ...
  if (Pars::icutoff == 0)
  {
    q = 1.0e0 / (2.0e0 * Pars::pi);
  }
  else if (Pars::icutoff == 1)
  {
    q = ((rho2 * (rho2 + 2.0e0)) / pow((rho2 + 1.0e0), 2)) / (2.0e0 * Pars::pi);
  }
  else if (Pars::icutoff == 2)
  {
    qt = -rho2 / (2.0e0);
    q = (1.0e0 - std::exp(qt)) / (2.0e0 * Pars::pi);
  }
  else if (Pars::icutoff >= 3)
  {
    qt = -rho2 / (2.0e0);
    qte = (1 - qt) * std::exp(qt);
    q = (1.0e0 - qte) / (2.0e0 * Pars::pi);
  }
}
